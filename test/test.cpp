...
TEST_F(GCHeapTest, Efficieny_StressTest) {
...

    {
        ...
        cout << "Reference Counting Efficiency: " << rc_time / num_objects << endl;
    }
    heap.reset();
    {
        std::vector<void*> ptrs;
        ...
        gc.ms_collect(&heap);
        auto t1 = std::chrono::high_resolution_clock::now();
        ms_time = std::chrono::duration_cast<std::chrono::microseconds>(t1 - t0).count();

        cout << "Mark and Sweep Efficiency: " << ms_time / num_objects << endl;
    }
